...
export default function NewAdminPage() {
  ...
  const fetchRoles = async () => {
    try {
      ...
      if (data.success) {
        setRoles(data.data);
        const adminRole = data.data.find((role: Role) => role.name.toLowerCase() === "admin");
        if (adminRole) {
          setFormData((prev) => ({ ...prev, userRoleId: adminRole.id }));
        }
      }
    } catch (err) {
  ...
  return (
    <div className="container mx-auto p-8">
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl">
        <div className="bg-white rounded-lg shadow-sm border border-gray p-6 space-y-6">
          <div>
